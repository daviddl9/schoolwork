...
class LabThree {
  ...
  public static void main(String[] args) {
    ...
    test(() -> InfiniteListGrader
        .iterate(1, x -> x + 1)
        .filter(x -> x % 2 == 0)
        .findFirst(x -> x >= 10),
        Optional.of(10),
        9,
        "iterate(1,x->x+1).filter(x%2==0).findFirst(x >= 10)");

    test(() -> InfiniteListGrader.iterate(1, x -> x + 1).takeWhile(x -> x <= 5).count(),
        5,
        5,
        "iterate(1,x->x+1).takeWhile(x<=5).count()");
  }
}
